...
all_items = bakery_items + food_items + bar_drink_items + bottle_drink_items + wholesale_items + coffee_items + miir_items
...
def get_learnable_data(df, item, bootstrap=False):
    X = np.asarray(df.iloc[:, 0:4]).reshape(-1, 4)
    y = np.asarray(df[item])
...
def get_cleaned_augmented_df_from_raw_csv():
        ...
        date_data =  date_utils.get_augmented_date_a(pd.DataFrame(df_daily_sums['Date'], columns = ['Date']))
